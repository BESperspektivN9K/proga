...
########################################################
    ...
    ws_t0.merge_cells('K4:S4')
    ws_t0['K3'] = f"Количество слов, выделенное под параметры 20Т0: {count_words} ({number_word-count_words} - {number_word-1})"
    ws_t0['K4'] = f"Количество счетчиков, использованных для распределения параметров 20Т0:  {max_counter}"
#######################################################################
    ...
    red_fill = PatternFill(start_color="FF9999", end_color="FF9999", fill_type="solid")
    for row in rows_errors_raz:
        ws_errors_raz.append([
            row["name"],
            row["id"],
            "",
            row["uchastok"],
        ...
        last_row = ws_errors_raz.max_row
        for col in range(1, 10):
            ws_errors_raz.cell(row=last_row, column=col).fill = red_fill
